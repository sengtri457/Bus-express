...
- **FR-CON-01 (Manifest Management)**: View assigned trip manifest and filter passenger listings by _Waiting_, _Boarded_, or _All_.
...
- **NFR-SEC-01 (Row-Level Security)**: Database security enforced directly inside Supabase PostgreSQL. Users can query data _only_ allowed by their specific RLS profile policy.
  - Passengers can view only _their own_ bookings and tickets.
  - Drivers/Conductors can read only _their own_ assigned trips and associated bookings.
